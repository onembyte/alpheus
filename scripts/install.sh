...
if [ "$INSTALLED" -eq 0 ]; then
    ...
    if [ -f "$LOCAL_CARGO" ]; then
        ...
        (cd "$SCRIPT_DIR/../src-tauri" && cargo build --release --bin alpheus-cli)
        install -m755 "$SCRIPT_DIR/../src-tauri/target/release/alpheus-cli" "$INSTALL_DIR/alpheus"
        INSTALLED=1
        ...
        (cd "$TMP_DIR/alpheus-src/src-tauri" && cargo build --release --bin alpheus-cli)
        install -m755 "$TMP_DIR/alpheus-src/src-tauri/target/release/alpheus-cli" "$INSTALL_DIR/alpheus"
        INSTALLED=1
    else
